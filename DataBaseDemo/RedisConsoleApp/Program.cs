...
namespace RedisConsoleApp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            /*
            安装步骤：
             1. 在你下载好的 redis on windows 项目, 找到bin目录, 里面会有redis-2.8.9.zip, 
             把这个解压到某个地方.
             2. 用 Windows command 进入到解压的目录，运行 redis-server --service-install 
             把 redis server 安装成 window service来运行，这个service默认监听端口是6379, 
             然后启动服务. (更简单的做法是直接打开这个目录,双击运行redis-server.exe)
